...
// @ts-ignore
const leerBDD = new Promise((resolve, reject) => {
...
leerBDD
    ...
    contenidoArchivo.forEach((actual, indiceActual, arreglo) => {
        if (actual['gender'] === 'male')
            genderM.push(actual['gender']);
        else if (actual['gender'] === 'female')
            genderF.push(actual['gender']);
        else
            genderNA.push(actual['gender']);
    });
...
leerBDD
    ...
    let othersEyes = [];
    contenidoArchivo.forEach((actual, indiceActual, arreglo) => {
        if (actual['eye_color'] === 'blue')
            blueEyes.push(actual['eye_color']);
        else if (actual['eye_color'] === 'yellow')
            yellowEyes.push(actual['eye_color']);
        else if (actual['eye_color'] === 'red')
            redEyes.push(actual['eye_color']);
        else if (actual['eye_color'] === 'brown')
            brownEyes.push(actual['eye_color']);
        else
            othersEyes.push(actual['eye_color']);
    });
